 ...
 * 
 * プロトコルUNITでのみ使えるバッファリング用パラメータクラス
 */
 ...
 * プロトコルUNITでのみ使えるバッファリング用パラメータクラス
 */
class BufferingProtocol implements IProtocolParameter
{
    //--------------------------------------------------------------------------
    // 定数
    //--------------------------------------------------------------------------
    ...
    final public function setReceivingSize(int $p_size)
    {
        ...
        $w_ret = $this->manager->setBufferingReceivingSize($cid, $p_size);
        if($w_ret === false)
        {
    ...
    {
        $cid = $this->param->getConnectionId();
        $w_ret = $this->manager->bufferingRreceiving($cid);
        if($w_ret === false)
        {
            throw new UnitException(
    ...
    final public function recv(&$p_recv, ?int $p_size = null): int
    {
        // データ受信
        $cid = $this->param->getConnectionId();
        $w_ret = $this->manager->bufferingRecv($cid, $p_recv, $p_size);
        if($w_ret === false)
        {
            throw new UnitException(
                UnitExceptionEnum::ECODE_RECEIVING_FAIL->message(),
