...
SOCKET-MANAGER Library は、WebSocket 90,000 接続を 30 秒で処理する「ハイパフォーマンスモード（純粋性能）」に加え、CPU割当なし・メモリ128MBで 100,000 同時接続を維持する「実運用スケールベンチマーク（実運用性能）」も公開しています。

高スループット・低レイテンシ・軽量性・堅牢性の四要素をPHP 単体で満たす通信基盤として設計されています。
...
## 【 実運用スケールベンチマーク（Real-world Scale Benchmark） 】

SOCKET-MANAGER Library では、CPU割当なし（プロセスの CPU ピニングなし）・
...
| **実運用スケール性能** | CPU割当なし・128MBで 100,000 同時接続を維持する堅牢なスケール特性 |
...
- **実運用での堅牢性**：CPU割当なし・128MB環境でも 100,000 同時接続を維持し、破綻しないスケール特性を確認
