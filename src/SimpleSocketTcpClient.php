...
/**
 * シンプルソケットクラス（TCP Client 用）
 * 
 * ソケットリソースの管理と周期ドリブンの制御を行う
 */
final class SimpleSocketTcpClient implements ISimpleSocketTcpClient
{
    //--------------------------------------------------------------------------
    // 定数（ソケット関連エラーコード）
    //--------------------------------------------------------------------------
    ...
    private function create(): void
    {
        ...
        for($i = 0; $i < $max; )
        {
            $w_ret = @socket_connect($soc, $this->host, $this->port);
            if($w_ret === true)
            {
                break;
            }
            usleep($this->retry_interval);
            $i++;
        }
        if($i >= $max)
        {
